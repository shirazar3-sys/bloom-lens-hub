...
import { Download, Heart, Share2, Grid, LayoutGrid, X, ChevronLeft, ChevronRight } from "lucide-react";
    ...
    <div className="min-h-screen bg-background">
        ...
        <div className="container mx-auto px-6">
          <div className="mb-8">
            ...
            <div className="flex items-center gap-2 mt-6">
              <span className="text-sm text-muted-foreground me-2">{t.gallery.view}</span>
              <Button variant={gridSize === "large" ? "secondary" : "ghost"} size="icon" onClick={() => setGridSize("large")}>
                <LayoutGrid className="w-4 h-4" />
              </Button>
              <Button variant={gridSize === "small" ? "secondary" : "ghost"} size="icon" onClick={() => setGridSize("small")}>
                <Grid className="w-4 h-4" />
              </Button>
              {favorites.size > 0 && (
                <span className="ms-4 text-sm text-primary font-medium">
                  {favorites.size} {t.gallery.favoritesSelected}
                </span>
              )}
            </div>
          </div>

          <div className={`grid gap-4 ${gridSize === "large" ? "grid-cols-1 md:grid-cols-2 lg:grid-cols-3" : "grid-cols-2 md:grid-cols-3 lg:grid-cols-4"}`}>
            {photos.map((photo, index) => (
              ...
              >
                <img src={photo.src} alt={photo.title} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" style={{ aspectRatio: photo.orientation === "portrait" ? "3/4" : photo.orientation === "landscape" ? "4/3" : "1/1" }} />
                ...
                <div className="absolute bottom-0 left-0 right-0 p-4 flex items-center justify-between opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                  <span className="text-background text-sm font-medium">{photo.title}</span>
                  <Button variant="ghost" size="icon" className="text-background hover:bg-background/20" onClick={(e) => toggleFavorite(photo.id, e)}>
      ...
      {selectedPhoto !== null && currentPhoto && (
        <div className="fixed inset-0 z-50 bg-foreground/95 flex items-center justify-center" onClick={closeLightbox}>
          <Button variant="ghost" size="icon" className="absolute top-4 end-4 text-background hover:bg-background/10" onClick={closeLightbox}>
            <X className="w-6 h-6" />
          </Button>
          <Button variant="ghost" size="icon" className="absolute start-4 top-1/2 -translate-y-1/2 text-background hover:bg-background/10" onClick={(e) => { e.stopPropagation(); navigatePhoto("prev"); }}>
          ...
          <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-4">
            ...
            <Button variant="ghost" size="sm" className="text-background hover:bg-background/10 gap-2">
